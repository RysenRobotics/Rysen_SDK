...
./install_rysen_deps.sh
```
This script automatically installs all required dependent libraries. If some dependencies are already installed, the script will skip them.
...
All example programs use Ethernet connection by default: (Ensure the local IP address is in the same network segment as the device address: 192.168.0.xxx)
