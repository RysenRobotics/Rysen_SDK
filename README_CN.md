...
在开始之前，请确保您的宿主机系统满足以下软硬件要求。Rysen Explorer 通过 Docker 部署，**无需**在本地安装 Python、C++ 编译环境或 ROS2；运行时依赖已包含在预构建镜像中。

### 硬件配置

| 项目 | 要求 |
|------|------|
| **以太网** | 主机至少一个可用网口；推荐千兆，与灵巧手同网段 |
| **连接方式** | 网线直连或经低延迟交换机；避免高延迟 Wi‑Fi |
| **设备端口** | 端口1 **5856**、端口2 **5857**（TCP） |
...
| **架构** | x86_64（CPU） |
| **Docker Engine** | ≥ **20.10** |
| **Docker Compose** | ≥ **2.0**（Compose V2 插件，使用 `docker compose` 命令） |
...
- 主机可访问灵巧手 IP（`ping` 通）
- 防火墙放行 TCP **5856**、**5857**
- 控制/数据口 RTT：普通内核 ≤ **5 ms**，实时内核（PREEMPT_RT）≤ **1 ms**

> 详细网段与宿主机 IP 配置步骤见下方 **[网络连接配置](#-连接配置)** 章节。
